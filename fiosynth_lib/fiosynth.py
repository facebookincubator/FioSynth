...
def set_attributes():
    ...
    parser = argparse.ArgumentParser(
        description='FB fio Synthetic Benchmark Suite for storage ver 3.5.38'
    )
    parser.add_argument(
        '-d',
        action='store',
    ...
    parser.add_argument(
        ...
        dest='misc',
        type=str,
        help='(Optional) Set a misc variable in a workload suite'
        '(default = )',
        default='',
    )
    parser.add_argument(
...
def getMultiplier(capacity):
    ...
    return max(round(factor, 1), .1)


def getJobVars(dut, profile, scale_factor):
    vars_static = [
        "SIZE", "TIME", "RAMPTIME", "RATE1", "RATE2", "BLKSIZE", "DEPTH1",
        "DEPTH2", "MISC"
    ]
    ...
    for var in vars_scaleup:
        if var in profile:
            if var == 'OFFSET2':
                job_vars['OFFSET2'] = int(profile['OFFSET2']
                    * getMultiplier(dut.capacity))
            else:
...
def run_fio(p, VAL, dut_list, args, run, rtype):
    ...
    resultsFileName = '%s/%s_run%d.json' % (
        FioDUT.fname, p[rtype][VAL]['alias'], run
    )
    ...
    for dut in dut_list:
        ...
        if dut.inLocalMode():
            ...
            for var in jobVars.keys():
                tmpJbStr = tmpJbStr.replace(
                    '${%s}' % str(var), str(jobVars[var])
                )
...
def clearDriveData(dut_list, dryrun='n'):
    ...
    if dut_list[0].inLocalMode():
        cmd = 'fio --name=trim --filename=%s --rw=trim --bs=1G' \
            % (dut_list[0].device)
    else:
        cmd = 'fio --name=trim --rw=trim --bs=1G '
        for dut in dut_list:
            cmd = cmd + '--client=ip6:%s --filename=%s' % (
                dut.serverName, dut.device
            )
    if dryrun == 'n':
        cmdline(cmd)
...
def runTest(dut_list, profile, args, csvFolderPath, rtype, index, rcycle):
    ...
    if args.dryrun == 'n':
        if dut_list[0].inLocalMode():  # Health tools only works locally
            ...
            results = Parser(
                jfile, '%s/%s.csv' % (FioDUT.fname, FioDUT.fname)
            )
        ...
        if 'pre' in profile and pc > 0 and lp > 0:
            ...
            for _pcycle in range(1, pc + 1):
                for index in range(len(profile['pre'])):
                    ...
                    runTest(dut_list, profile, args, csvFolderPath, 'pre', index,
                                                                        rcycle)
        for index in range(len(profile['def'])):
            runTest(dut_list, profile, args, csvFolderPath, 'def', index,
                                                                    rcycle)
