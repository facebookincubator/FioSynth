...
def set_attributes():
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            '(Optional) Flash configuration filename (default = '
            'flashconfig.csv'
        ),
...
def smartctlToJson(data):
    ...
    while index != len(data[device]):
        ...
        for line in lb.stdout:
            bits = line.split()
            a = '%s %s' % (
                bits[KEY].decode("utf-8"), bits[KEY + 1].decode("utf-8")
            )
            try:
                b = '%s %s' % (
...
def new_csv(f):
    try:
        col_names = []
        col_names = [
            'Index', 'DevicePath', 'Capacity', 'ModelNumber', 'SerialNumber',
            'Firmware', 'Hostname', 'KernelVersion'
        ]
        writer = csv.writer(f)
...
def print_nvme_line(f, data, hostname, kernel):
    ...
    for datum in data:
        ...
        try:
            writer = csv.writer(f)
            row = (
                datum['Index'], datum['DevicePath'], capacity,
                datum['ModelNumber'], datum['SerialNumber'], datum['Firmware'],
                hostname, kernel
            )
            writer.writerow(row)
        except IOError:
...
def print_flash_line(f, data, hostname, kernel):
    ...
    try:
        ...
        writer.writerow(
            (
                data[card_id]['pci_address'], data[card_id]['logical_location'],
                data[card_id]['size'], data[card_id]['board_name'],
                data[card_id]['sn'], data[card_id]['firmware_version'],
                hostname, kernel
            )
        )
    except IOError:
    ...
    while index - 1 <= len(data[KEY]):
        ...
        if (data[sidx]['Rotation Rate:'] == 'Solid State'):
            try:
                ...
                writer.writerow(
                    (
                        index, data[sidx]['Device Path:'],
                        data[sidx]['User Capacity:'],
                        data[sidx]['Device Model:'],
                        data[sidx]['Serial Number:'],
                        data[sidx]['Firmware Version:'], hostname, kernel
                    )
                )
            except IOError:
                print('cannot write to ', f)
